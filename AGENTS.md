...
modules/                                   # ALL files below are flake-parts modules, auto-loaded by import-tree
  ...
  users/
    home-manager.nix                       # user-agnostic coupling: nixos.base imports home-manager + declares my.homeUsers; nixos.{base,desktop,development} attach homeManager.{base,desktop,development} to every registered user
    ...
    lansing.nix                            # lansing user aspect: nixos.user-lansing (account, groups, SSH key, sudo, GECOS) self-registers into my.homeUsers; homeManager.lansing = username/homeDirectory
  system/                                  # → nixos.base
    ...
    sops.nix                               # sops-nix wrapper: defaultSopsFile + 4 secrets owned by lansing
  ...
  development/                             # → nixos.development + homeManager.development
    ...
    docker.nix                             # virtualisation.docker (lansing in modules/users/lansing.nix's groups)
...
- Aspects contributing `lansing.desktop.niri.appWindowRules` (slack, vesktop, signal, onepassword, steam) implicitly require the host to also import `desktop` (which declares the option). True for both current hosts; keep the invariant for future hosts.
...
### Add a user
A user is one aspect: a new `modules/users/<name>.nix` defining `flake.modules.nixos.user-<name>` (the OS account + `my.homeUsers = [ "<name>" ]` self-registration + `home-manager.users.<name>.imports = [ <their homeManager bucket> ]`) and `flake.modules.homeManager.<name>` (username/homeDirectory + personal home config). A host gains the user by adding `user-<name>` to its `with config.flake.modules.nixos; [ … ]` list in `modules/hosts/<host>.nix`; the base + role home buckets (`base`/`desktop`/`development`) attach to every registered user automatically. Different users per host = each host imports its own `user-<name>`; several users on one host = import several. Shared, user-agnostic home defaults belong in `modules/users/home-base.nix`, never in a per-user bucket.
...
- **Docker group membership lives in `modules/users/lansing.nix`**, not in `modules/development/docker.nix`. Keeping all of `lansing`'s extraGroups in the user aspect avoids "is the user in the right groups?" hunting across files.
