# Sunshine = Moonlight game-streaming host. battlestation only.
{
  flake.modules.nixos.sunshine = { config, lib, pkgs, ... }: {
    # Streams the desktop and game sessions to any Moonlight client (TV,
    # phone, laptop) over LAN or Tailscale. The `services.sunshine` module
    # in nixpkgs already wires the firewall, the CAP_SYS_ADMIN wrapper,
    # /dev/uinput udev rules, the avahi mDNS announcement, and the systemd
    # user unit tied to graphical-session.target. We only feed it settings,
